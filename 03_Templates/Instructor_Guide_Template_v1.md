# Instructor Guide Template v1

## Naznachenie

Shablon nuzhen dlya sborki rabochey metodichki ili `PDF` dlya lektora, kotoryy dolzhen podgotovitsya k provedeniyu zanyatiya po gotovoy prezentatsii.

## Pravilo formata

- dokument dolzhen byt udoben dlya pechati i eksporta v `PDF`;
- kazhdaya stranitsa dolzhna ostavlyat mesto pod opzionalnyy `QR`-slot;
- `QR` ne dolzhen byt obyazatelnym dlya tekushchey versii, no struktura dolzhna podderzhivat ego dobavlenie bez peresborki vsego fayla;
- esli resurs budet ispolzovat'sya v on-line, nuzhno predusmatrivat `direct link` kak paru k QR.

## Obshchaya informatsiya

- Nazvanie kursa:
- Kod kursa:
- Versiya:
- Dlya kakogo release / klienta:
- Owner:
- Format vydachi: `PDF`

## QR-ready policy

- Standardnoe raspolozhenie QR:
- Chto mozhet byt zakodirovano: `video / skhema / dokument / FAQ / demo`
- Est li v etoy versii aktivnye QR:
- Est li rezervnye QR-sloty bez ssylki:
- Est li direct links dlya on-line versii:

## 0. Delivery profile dlya lektora

- Track auditorii: `worker / ITR / mixed`
- Tselevoi format provedenia: `Short / Standard / Deep`
- Razmer profilya: `Short = do 60 + test / Standard = 80-90 + test / Deep = 110-120 + test`
- Rezhim: `with-test / no-test`
- Delivery mode: `instructor-led / blended / online-support`
- Chto obyazatelno ne poteryat pri etom profile:

## 0a. Teaching paths po auditorii

| Auditoriia | Na chem delat glavniy aktsent | Chto ne peregruzhat | Kakie voprosy veroyatny |
|---|---|---|---|
| Worker |  |  |  |
| ITR |  |  |  |
| Mixed |  |  |  |

## 1. Pasport kursa

- Dlya kogo kurs:
- Chto obyazatelno ponyat lektoru:
- Chto kurs pokryvaet:
- Chto kurs ne pokryvaet:
- Kakoy rezultat dolzhen byt u slushatelya:

## 2. Bystryy vkhod v professiyu

- Chem zanimaetsya spetsialist:
- Iz kakikh tipovykh blokov sostoit rabota:
- Gde glavnye riski:
- Kakie oshibki samye opasnye:
- Chto lektoru nado ponyat pered pervym zanyatiem:

## 3. Normativnyy minimum

| Dokument | Zachem nuzhen lektoru | Chto iz nego obyazatelno znat | Gde v kurse ispolzuetsya |
|---|---|---|---|
|  |  |  |  |

## 4. Stsenariy zanyatiya

| Blok | O chem govorit lektor | Na chto sdelat aktsent | Interaktiv / vopros gruppe | Primechanie |
|---|---|---|---|---|
|  |  |  |  |  |

## 5. Tayming

| Versiya | Vremya | Primechanie |
|---|---|---|
| Short |  |  |
| Standard |  |  |
| Deep |  |  |

## 5a. Coverage control

- Aktualnyy diapazon `slide script`:
- Pokryto v etoy versii guide:
- Est li slaydy bez lektorskikh notes:
- Reshenie po pokrytiyu: `full / partial`

## 6. Poslaydovye poyasneniya

| Slayd | Glavnaya mysl | Chto skazat obyazatelno | Tipichnyy vopros ili risk | QR-slot |
|---|---|---|---|---|
| 01 |  |  |  | `QR-01 reserved / linked` |

## 7. FAQ dlya lektora

| Vopros | Korotkiy otvet | Rasshirennyy otvet | Kogda nuzhna ogovorka |
|---|---|---|---|
|  |  |  |  |

## 7a. Slozhnye voprosy i granitsy

- Nazvanie svyazannogo fayla ili banka voprosov:
- Est li otdelnye worker-voprosy:
- Est li otdelnye ITR-voprosy:
- Est li voprosy, gde nuzhen pasport konkretnoy sistemy ili lokalnyy reglament:

## 8. Video i materialy dlya podgotovki

| ID | Tip | Chto posmotret ili prochitat | Zachem eto nuzhno | QR-slot |
|---|---|---|---|---|
| M-01 | video / document / scheme |  |  | `QR-M01 reserved / linked` |

## 9. Cheklist podgotovki lektora

- Izuchen pasport kursa
- Prochten normativnyy minimum
- Ponyata logika professii
- Prosmotreny video i materialy
- Poveren tayming
- Otkryty FAQ i otvety na veroyatnye voprosy
- Otkryt otdelnyy `lecturer questions bank`
- Poveren format `with-test / no-test`
- Povereno, chto QR-sloty ne meshaut chteniyu stranitsy

## Reshenie po gotovnosti

- Instructor guide gotov k eksportu v `PDF`: `yes / no`
- Pokrytie `slide script` polnoe: `yes / no`
- Chto nado dosobrat:
